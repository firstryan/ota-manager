...
const uploadToAppStore = rawArgs.includes('--upload') || rawArgs.includes('-u') || rawArgs.includes('upload') ? 'true' : 'false';

const cleanArgs = rawArgs.filter(a => !a.startsWith('-') && a !== 'nover' && a !== 'no-ver' && a !== 'no' && a !== 'norev' && a !== 'upload');
    ...
    if ((activeRuns.workflow_runs && activeRuns.workflow_runs.length > 0) || (queuedRuns.workflow_runs && queuedRuns.workflow_runs.length > 0)) {
        ...
        try {
            ...
            payload.inputs = {
                ...
                upload_to_app_store: uploadToAppStore
            };
