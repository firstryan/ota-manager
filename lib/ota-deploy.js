    ...
    try {
        ...
        try {
            ...
            if (fs.existsSync(FLATTEN_SCRIPT)) {
                console.log('🚀 Running Absolute Path Normalization...');
        ...
        if (argChannel === 'live' && fs.existsSync(envLivePath)) {
            ...
            console.log(`📝 Updated .env-live to version ${nextVersion}`);
        }
        if (argChannel === 'training' && fs.existsSync(envTrainingPath)) {
            ...
            console.log(`📝 Updated .env-training to version ${nextVersion}`);
        }
        ...
        try {
            ...
            console.log(`✅ Native Android & iOS versions synchronized (versionCode: ${versionCodeNum}).`);
        } catch (err) {
            console.error('⚠️ Warning: Failed to sync native versions:', err.message);
        }
        ...
        console.log('🏗️ Rebuilding Astro to bake new version string into JS bundle...');
        hideApi();
        try {
            execSync('npm run build', { stdio: 'inherit', cwd: rootDir });
