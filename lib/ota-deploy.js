...
async function deployOTA() {
    ...
    try {
        ...
        if (!nextVersion) {
            console.log('🔍 No version provided, auto-incrementing from remote manifest...');
            ...
            if (fs.existsSync(otaManifestPath)) {
                ...
                console.log(`📈 Auto-increment: ${latestVersion} -> ${nextVersion}`);
            } else {
                nextVersion = currentVersion;
                console.log(`⚠️ No remote manifest, using .env version: ${nextVersion}`);
            }
        }
        ...
        console.log('🏗️ Building project for OTA...');
        hideApi();
        try {
