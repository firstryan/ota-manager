...
async function run() {
    try {
        switch (command) {
            ...
            case 'kill-task':
                // Read lock file and terminate any running OTA build
                const lockPath = path.join(__dirname, '..', '.ota-build.lock');
                if (fs.existsSync(lockPath)) {
                    const pid = parseInt(fs.readFileSync(lockPath, 'utf8'), 10);
                    if (pid && pid !== process.pid) {
                        try {
                            process.kill(pid);
                            console.log(`⚡️ Killed OTA build task with PID ${pid}`);
                        } catch (e) {
                            console.warn(`⚠️ Unable to kill PID ${pid}: ${e.message}`);
                        }
                    }
                    fs.unlinkSync(lockPath);
                    console.log('🗑️ OTA build lock removed.');
                } else {
                    console.log('✅ No OTA build lock found – no running task.');
                }
                process.exit(0);
                break;
            ...
            case 'set-version':
                if (!subArg) {
                    console.log('❌ Error: Mohon tentukan versi tujuan (e.g., npx ota-manager rollback 0.2.0.11 training).');
                    process.exit(1);
                }
